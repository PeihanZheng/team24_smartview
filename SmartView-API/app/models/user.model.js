...
Users.create = (newUser, result) => {
  sql.query("INSERT INTO mydb.users SET ?", newUser, (err, res) => {
    if (err) {
...
Users.findById = (username, result) => {
  sql.query(
    `SELECT * FROM mydb.users WHERE username = "${username}"`,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
...
Users.getAll = (result) => {
  sql.query("SELECT * FROM mydb.users", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Users.remove = (username, result) => {
  ...
  sql.query(
    "DELETE FROM mydb.users WHERE username = ?",
    username,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Users.update = (name, user, result) => {
  sql.query(
    "UPDATE mydb.users SET username = ?, phone_number = ?, email_address = ? WHERE name = ?;",
