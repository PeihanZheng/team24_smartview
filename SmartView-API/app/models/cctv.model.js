...
CCTV.findById = (cctv_id, result) => {
  sql.query(
    `SELECT * FROM mydb.cctv WHERE cctv_id = ${cctv_id}`,
    (err, res) => {
      if (err) {
...
// Find all CCTVs
CCTV.getAll = (result) => {
  sql.query("SELECT * FROM mydb.cctv", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
CCTV.create = (newCCTV, result) => {
  sql.query("INSERT INTO mydb.cctv SET ?", newCCTV, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
CCTV.updateById = (id, cctv, result) => {
  sql.query(
    "UPDATE mydb.cctv SET cctv_name = ?, cctv_link_address = ?, cctv_physical_address = ? WHERE cctv_id = ?;",
...
CCTV.remove = (id, result) => {
  sql.query("DELETE FROM mydb.cctv WHERE cctv_id = ?;", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
CCTV.findByIncidentId = (incident_id, result) => {
  sql.query(
    `SELECT cctv_physical_address FROM mydb.cctv INNER JOIN mydb.incidents ON cctv.cctv_id = incidents.cam_id WHERE incidents.incident_id = ${incident_id}`,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
...
CCTV.findLocationByIncidentId = (incident_id, result) => {
  sql.query(
    `SELECT cctv_latitude, cctv_longitude FROM mydb.cctv INNER JOIN mydb.incidents ON cctv.cctv_id = incidents.cam_id WHERE incidents.incident_id = ${incident_id}`,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
